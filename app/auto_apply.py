...
def auto_answer_questions(
    ...
    # First answer from questions list
    first_q = questions[0]
    opts = (
        list(first_q.get("answerOption", {}).values())
        if first_q.get("answerOption")
        else None
    )
    ...
    i = 1  # index for questions list fallback
    while True:
        print("🤖 Sending payload:", json.dumps(payload, indent=2))
        res = session.post(url, headers=headers, cookies=cookies, json=payload)
        data = res.json()
        print(f"🤖 Bot response: {json.dumps(data,indent=2)}")

        if not data.get("speechResponse"):
            break

        speech = data["speechResponse"][0]["response"]
        print(f"🧠 Bot Question: {speech}")

        if data.get("isLeafNode") and speech == "Thank you for your responses.":
            print("✅ Final question acknowledged.")
            break

        opts = (
            [opt["value"] for opt in data.get("options", [])]
            if data.get("options")
            else None
        )
        answer = infer_answer(speech, opts)

        # fallback question ID from provided list if exists
...
def answer_questions(
    ...
    for q in questions:
        ...
        opts = list(q["answerOption"].values()) if q.get("answerOption") else None
        ...
        payload = {
            ...
            "applyData": {
                app_name.split("_")[0]: {"answers": {qid: [answer] if opts else answer}}
            },
    ...
    final_res = session.post(final_url, headers=headers, cookies=cookies, json=payload)
    print("🚀 Final apply call response:", final_res.status_code, final_res.text[:300])
...
def make_final_apply(
    ...
    print(f"📤 Sending final apply request for Job ID: {job_id}", json.dumps(payload))
    ...
    print(f"❌ Failed to make final apply for {job_id}. Status: {response.status_code}")
